...
1. **Financial Research Agents:** A user asks, "Should I invest in Tesla based on their last earnings call and current EV market trends?" 
    * *Decomposition:* 1. Fetch Tesla's Q3 transcript. 2. Fetch EV market reports. 3. Analyze transcript for sentiment. 4. Compare sentiment against market trends. 5. Draft recommendation.
    * *Routing:* Steps 1 & 2 go to a Web Browsing Agent. Steps 3 & 4 go to a Data Analysis Agent. Step 5 goes to a Financial Writing Agent.
2. **Customer Support Triage:** A user sends a long, rambling email complaining about a billing error and asking for a feature request. The orchestrator decomposes this into two tickets, routing the billing sub-task to the Finance Agent and the feature request to the Product Feedback database.
