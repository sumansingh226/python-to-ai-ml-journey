...
Context Caching: Controlling Cost
Context Compaction: Controlling Space
The Architectural Tension
The Golden Rule: Stable vs. Volatile Context
...
Cost Limit: You pay for all input tokens on every turn. If you send a 50K codebase + 50K history on every turn for 100 turns, you pay for 10M tokens.
Space Limit: The model physically cannot accept more tokens than its window. You get a hard context_length_exceeded error.
...
Write: You pay a small premium (e.g., 1.25x) to write a context block to the cache once. For example, you mark your system prompt + tool definitions + codebase as cacheable.
Read: As long as the prefix remains byte-for-byte identical on the next API call, you pay a fraction of the cost to read it (e.g., 0.1x). The computation is skipped.
TTL: The cache typically lives for 5-60 minutes and is refreshed on each hit.
Why it is used
...
Caching Everything: Putting the entire conversation history in the cache. Every turn invalidates it. No savings.
Late Compaction: Waiting until 95% full. Leads to truncated, low-quality summaries.
Freeform Summarization: "Summarize the conversation so far." Loses critical IDs, file paths, and decisions.
Hope-Based Memory: Assuming compaction will remember user preferences. It will not. Explicitly write to external memory.
Ignoring Breakpoints: Not using cache breakpoints, so a single changed character invalidates a 100K codebase cache.
